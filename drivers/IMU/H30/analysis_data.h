...
public:
    int calcChecksum(uint8_t *data, uint16_t len, uint16_t *checksum);
    int16_t getSignedShort(uint8_t *data);
    int32_t getSignedInt(uint8_t *data);
    int checkDataLenById(payload_data_t header, uint8_t *data, protocol_info_t *info);
    int analysisData(uint8_t *data, int16_t len, protocol_info_t *info);
